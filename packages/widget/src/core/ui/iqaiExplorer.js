...
const PRICE_CHART_RANGES = ['1M', '5D', '6M', '1Y', '5Y'];
...
export function createIqaiExplorerController(elements, options = {}) {
  ...
  const renderAgents = () => {
    ...
    elements.grid.innerHTML = list.map((agent) => {
      ...
      const rangeOptions = PRICE_CHART_RANGES
        .map((range) => `<span class="valki-iqai-range-option${range === PRICE_CHART_RANGE ? ' is-active' : ''}">${range}</span>`)
        .join('');

      return `<article class="valki-iqai-card"><div class="valki-iqai-body"><div class="valki-iqai-left"><div class="valki-iqai-head">${avatarMarkup}<div style="min-width:0;flex:1"><div class="valki-iqai-title-row"><div><h3 class="valki-iqai-title">${esc(agent.name)}</h3><div class="valki-iqai-ticker">${esc(agent.ticker || '-')}</div></div><button class="valki-iqai-btn valki-iqai-open-signal" data-open="${esc(agent.id)}" type="button">Traden</button></div><div class="valki-iqai-tags"><span class="valki-iqai-tag">${esc(agent.framework ?? '-')}</span><span class="valki-iqai-tag">Chain ${esc(agent.chainId ?? '-')}</span></div></div></div><div class="valki-iqai-bio">${esc(shortWords(agent.bio, 9))}</div><div class="valki-iqai-stats"><div>Holders: <strong>${esc(agent.holdersCount ?? '-')}</strong></div><div>Inference: <strong>${esc(agent.inferenceCount ?? '-')}</strong></div><div>Status: <strong>${agent.isActive ? 'Live' : 'Offline'}</strong></div><div>Verified: <strong>${agent.isVerified ? 'Yes' : 'No'}</strong></div></div></div><div class="valki-iqai-right"><div class="valki-iqai-chart-region"><div class="valki-iqai-chart-block"><div class="valki-iqai-price-wrap"><div class="valki-iqai-pair">${esc(pairLabel)}</div><div class="valki-iqai-card-price-row"><div class="valki-iqai-price">${price}</div><div class="valki-iqai-sparkline" data-ticker="${esc(ticker)}" role="img" aria-label="${esc(`${agent.ticker || 'Agent'} price trend`)}"></div>${hasPerformanceBadge ? `<span class="valki-iqai-performance ${performanceClass}">${esc(formatPercent(performancePercent))}</span>` : ''}</div></div></div><div class="valki-iqai-range">${rangeOptions}</div></div></div></div></article>`;
    }).join('');
