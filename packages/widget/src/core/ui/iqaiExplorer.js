...
function formatChangeBadge(value) {
  const num = Number(value);
...
export function createIqaiExplorerController(elements, options = {}) {
  ...
  const renderAgents = () => {
    ...
    elements.grid.innerHTML = list.map((agent) => {
      ...
      const performanceClass = performancePercent > 0 ? 'positive' : performancePercent < 0 ? 'negative' : 'neutral';
